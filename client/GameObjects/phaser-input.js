...
(function (PhaserInput) {
  ...
  var InputElement = (function () {
    function InputElement(game, id, type, value, focusIn, focusOut) {
      ...
      var canvasTopX =
        this.game.canvas.getBoundingClientRect().top + document.body.scrollTop;
    ...
    InputElement.prototype.focus = function () {
      ...
      if (!this.game.device.desktop && this.game.device.chrome) {
        ...
        var interval_1 = setInterval(function () {
          if (
            originalWidth_1 > window.innerWidth ||
            originalHeight_1 > window.innerHeight
          ) {
            kbAppeared_1 = true;
          }
...
(function (PhaserInput) {
  ...
  var InputField = (function (_super) {
    ...
    function InputField(game, x, y, inputOptions) {
      ...
      _this.inputOptions.width =
        typeof inputOptions.width === "number" ? inputOptions.width : 150;
      _this.inputOptions.padding =
        typeof inputOptions.padding === "number" ? inputOptions.padding : 0;
      ...
      _this.inputOptions.forceCase = inputOptions.forceCase
        ? inputOptions.forceCase
        : ForceCase.none;
      _this.inputOptions.borderRadius =
        typeof inputOptions.borderRadius === "number"
          ? inputOptions.borderRadius
          : 0;
      _this.inputOptions.height =
        typeof inputOptions.height === "number" ? inputOptions.height : 14;
      _this.inputOptions.fillAlpha =
        inputOptions.fillAlpha === undefined ? 1 : inputOptions.fillAlpha;
      _this.inputOptions.selectionColor =
        inputOptions.selectionColor || "rgba(179, 212, 253, 0.8)";
      _this.inputOptions.zoom = !game.device.desktop
        ? inputOptions.zoom || false
        : false;
      ...
      _this.domElement = new PhaserInput.InputElement(
        _this.game,
        "phaser-input-" + ((Math.random() * 10000) | 0).toString(),
        _this.inputOptions.type,
        _this.value,
        _this.focusIn,
        _this.focusOut
      );
      _this.domElement.setMax(_this.inputOptions.max, _this.inputOptions.min);
      _this.selection = new PhaserInput.SelectionHighlight(
        _this.game,
        _this.inputOptions
      );
      ...
      if (inputOptions.placeHolder && inputOptions.placeHolder.length > 0) {
        _this.placeHolder = new Phaser.Text(
          game,
          _this.inputOptions.padding,
          _this.inputOptions.padding,
      ...
      _this.cursor = new Phaser.Text(
        game,
        _this.inputOptions.padding,
        _this.inputOptions.padding - 2,
        "|",
        {
          font: inputOptions.font || "14px Arial",
          fontWeight: inputOptions.fontWeight || "normal",
      ...
      _this.offscreenText = new Phaser.Text(
        game,
        _this.inputOptions.padding,
        _this.inputOptions.padding,
        "",
      ...
      switch (this.inputOptions.textAlign) {
        ...
        case "center":
          ...
          if (null !== this.placeHolder) {
            ...
            this.placeHolder.x =
              this.inputOptions.padding + this.inputOptions.width / 2;
          }
          this.cursor.x =
            this.inputOptions.padding +
            this.inputOptions.width / 2 -
            this.text.width / 2 +
        ...
        case "right":
          ...
          if (null !== this.placeHolder) {
            ...
            this.placeHolder.x =
              this.inputOptions.padding + this.inputOptions.width;
          }
    ...
    InputField.prototype.updateCursor = function () {
      if (
        this.text.width > this.inputOptions.width ||
        this.inputOptions.textAlign === "right"
      ) {
        this.cursor.x = this.inputOptions.padding + this.inputOptions.width;
      } else {
        switch (this.inputOptions.textAlign) {
    ...
    InputField.prototype.setCaretOnclick = function (e) {
      var localX = this.text.toLocal(
        new PIXI.Point(e.x, e.y),
        this.game.world
      ).x;
      if (
        this.inputOptions.textAlign &&
        this.inputOptions.textAlign === "center"
      ) {
      ...
      for (var i = 0; i < this.value.length; i++) {
        if (
          localX >= i * characterWidth &&
          localX <= (i + 1) * characterWidth
        ) {
          index = i;
          break;
        }
    ...
    InputField.prototype.updateSelection = function () {
      if (this.domElement.hasSelection) {
        ...
        text = text.substring(
          this.domElement.caretStart,
          this.domElement.caretEnd
        );
    ...
    InputField.prototype.zoomIn = function () {
      ...
      var offsetX =
        (this.game.width - bounds.width * 1.5) / 2 / this.windowScale;
      this.game.world.scale.set(
        this.game.world.scale.x * this.windowScale,
        this.game.world.scale.y * this.windowScale
      );
      this.game.world.pivot.set(
        bounds.x - offsetX,
        bounds.y - this.inputOptions.padding * 2
      );
...
(function (PhaserInput) {
  var InputBox = (function (_super) {
    ...
    function InputBox(game, inputOptions) {
      ...
      _this.bgColor = inputOptions.backgroundColor
        ? parseInt(inputOptions.backgroundColor.slice(1), 16)
        : 0xffffff;
      _this.borderRadius = inputOptions.borderRadius || 0;
      _this.borderWidth = inputOptions.borderWidth || 1;
      _this.borderColor = inputOptions.borderColor
        ? parseInt(inputOptions.borderColor.slice(1), 16)
        : 0x959595;
      ...
      if (inputOptions.font) {
        height = Math.max(
          parseInt(
            inputOptions.font.substr(0, inputOptions.font.indexOf("px")),
            10
          ),
          height
        );
  ...
  var SelectionHighlight = (function (_super) {
    ...
    SelectionHighlight.prototype.updateSelection = function (rect) {
      ...
      this.drawRect(
        ...
        rect.y,
        rect.width,
        rect.height - this.inputOptions.padding
      );
    };
...
(function (PhaserInput) {
  ...
  var Plugin = (function (_super) {
    ...
    Plugin.prototype.addInputFieldFactory = function () {
      Phaser.GameObjectFactory.prototype.inputField = function (
        x,
        y,
        inputOptions,
        group
        ...
        var nineSliceObject = new PhaserInput.InputField(
          this.game,
          x,
          y,
          inputOptions
      ...
      Phaser.GameObjectCreator.prototype.inputField = function (
        x,
        y,
        inputOptions
      ) {
